#!/usr/bin/env python3
"""
Stage D: Visual RAG end-to-end. Image -> SigLIP image-tower embedding -> retrieval
against the corpus's SigLIP text embeddings -> Qwen3-VL diagnosis (same v2 prompt as the
main ablation, unchanged, for direct comparability). Same 465 primary cases, same corpus,
same evaluation metrics as every prior Phase 2 experiment. Only the retrieval
...
# identical to the v2 fix in run_phase2_ablation.py, for direct comparability
...
def main():
    ...
    print("loading diagnostic corpus + SigLIP text embeddings...")
    ...
    out_path = Path(args.out) if args.out else DATA_DIR / "visual_rag_diagnosis_results.jsonl"
    ...
    with open(out_path, "w") as out_f, open(error_path, "w") as err_f:
        for case in cases:
            ...
            try:
                ...
                q_emb = torch.nn.functional.normalize(feat.float(), dim=-1).cpu().numpy().astype("float32")[0]

                retrieved = retrieve(q_emb, corpus_emb, corpus, TOP_K)
